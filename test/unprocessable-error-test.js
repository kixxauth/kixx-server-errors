...
module.exports = function (t) {
	...
	t.it('should have a full stack trace', () => {
		...
		assert.isOk(firstLines[1].includes('test/unprocessable-error-test.js:'));
	});

	t.it('assigns the given pointer', () => {
		const undef = new ErrorClass('Foo bar baz.');
		const err = new ErrorClass('Foo bar baz.', {pointer: 'foobarbaz'});

		assert.isEqual('undefined', typeof undef.pointer);
		assert.isEqual('foobarbaz', err.pointer);
	});
};
